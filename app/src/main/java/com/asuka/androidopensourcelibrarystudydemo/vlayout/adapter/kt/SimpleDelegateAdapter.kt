...
abstract class SimpleDelegateAdapter<T,VB:ViewBinding>(open var dataList:MutableList<T>, open var layoutHelper: LayoutHelper)
    : DelegateAdapter.Adapter<RecyclerViewHolder<VB>>() {

    open lateinit var holder: RecyclerViewHolder<VB>

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerViewHolder<VB> {
        holder = RecyclerViewHolder(onCreateViewBinding(LayoutInflater.from(parent.context),parent,viewType))
        return holder
    }
